...
import {BrowserRouter as Route, Switch, useHistory, useParams, useLocation} from "react-router-dom"
import PlayList from "./playlist";

const My = ({mymusic, comment, setMusicUrl, setComment, setMymusic}) => {
    ...
    useEffect(() => {
        ...
        const Mymusic = async () => {
            ...
            const res1 = await fetch(`http://localhost:3000/user/playlist?uid=${data.profile.userId}&cookie=${cookie}`);
            const data1 = await res1.json();
            setMymusic(data1);
        }
        Mymusic();
