...
const PlayList = ({setMusicUrl, comment, margin, myId = null, myCookie = null}) => {
    ...
    const sendComment = async () => {
        const res = await fetch(`http://localhost:3000/comment?t=1&type=2&id=${play.id}&content=${contentText}&cookie=${cookie ? cookie : myCookie}`);
        const data = await res.json();
        setNewComment(data);
    }
    ...
    useEffect(() => {
        ...
        const login = async () => {
            const res = await fetch(`http://localhost:3000/login/status?cookie=${cookie ? cookie : myCookie}`, {
                credentials: "include",
                mode: "cors"
            });
            const data = await res.json();
