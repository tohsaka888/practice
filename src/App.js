...
function App() {
    ...
    useEffect(() => {
        ...
        const gedan = async () => {
            const res = await fetch(`http://localhost:3000/top/playlist?limit=12&cookie=${cookie}`, {mode: "cors"});
            const data = await res.json();
        ...
        const toplist = async () => {
            ...
            const res4 = await fetch("http://localhost:3000/toplist");
            const data4 = await res4.json();
            setToplist(data4.list.slice(0, 3));
            const res5 = await fetch(`http://localhost:3000/playlist/detail?id=${data4.list[0].id}`);
            const data5 = await res5.json();
            setToplistSongs(data5.playlist.tracks.slice(0, 10));
            const res6 = await fetch(`http://localhost:3000/playlist/detail?id=${data4.list[1].id}`);
            const data6 = await res6.json();
            setToplistSongs1(data6.playlist.tracks.slice(0, 10));
            const res7 = await fetch(`http://localhost:3000/playlist/detail?id=${data4.list[2].id}`);
            const data7 = await res7.json();
            setToplistSongs2(data7.playlist.tracks.slice(0, 10));
        }
        const login = async () => {
            const res = await fetch(`http://localhost:3000/login/status?cookie=${cookie}`, {
                credentials: "include",
                mode: "cors"
            });
            const data = await res.json();
            await fetch(`http://localhost:3000/login/refresh?cookie=${cookie}`, {credentials: "include", mode: "cors"});
            setLoginState(data);
        }
        const newAlbum = async () => {
            const res = await fetch(`http://localhost:3000/top/album?limit=30&cookie=${cookie}`, {
                mode: "cors",
                credentials: "include"
            });
            const data = await res.json();
        ...
        const like = async () => {
            const res = await fetch(`http://localhost:3000/recommend/resource?cookie=${cookie}`, {
                mode: "cors",
                credentials: "include"
            });
            const data = await res.json();
                ...
                <Router>
                    <Layout>
                        ...
                        <Switch>
                            ...
                            <Route path={`/mymusic/:id/:cookie`}><My mymusic={mymusic} comment={comment}
